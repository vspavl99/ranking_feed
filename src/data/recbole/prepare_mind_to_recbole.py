import click
import importlib
from src.data.recbole.RecDatasets.conversion_tools.src.utils import dataset2class
...
def create_recbole_dataset(dataset, input_path, output_path):

    input_args = [input_path, output_path]
    dataset_class_name = dataset2class[dataset.lower()]
    dataset_class = getattr(
        importlib.import_module('src.data.recbole.RecDatasets.conversion_tools.src.extended_dataset'), dataset_class_name
    )
